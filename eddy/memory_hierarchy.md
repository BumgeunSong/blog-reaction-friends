...
> 네트워크 상에서 '캐싱'을 제공해주는 사업은 시장 크기가 무려 10조원이 넘는다. 사람들이 인터넷 상에서 점점 더 용량이 큰 이미지, 영상, 게임을 소비한다. 이런 콘텐츠 데이터를 빠르고, 효율적으로 제공하기 위해서 캐싱은 필수다. 

> 이를 위해서는 콘텐츠 소비자와 더 가까운 곳에 캐시 역할을 하는 원격 서버가 있어야 하는데, 이런 캐시 서버를 제공하는 서비스를 Content Delivery Network(CDN)이라고 한다. 우리가 잘 모르지만 인터넷 인프라의 중요한 한 축을 담당하고 있다. 

> 미국에 있는 구글 데이터 센터에서 영상을 전송하면, 해저 케이블을 건너 한국까지 와야한다. 하지만 한국이나 일본에 데이터 센터를 두고 데이터를 복사해둔다. 그쪽에서 데이터를 바로 전달받으면 네트워크 사용량도 적고 훨씬 빠르다. 네트워크의 병목이나 서버 부하도 줄여줄 수 있다. 글로벌 서비스를 하는 회사 입장에서는 큰 메리트다.

> CDN 회사들은 전세계에 데이터 센터를 확보해두고, 서비스 회사들에게 인프라를 빌려주는 대신 돈을 번다. 

### 캐시 관리 방법

캐시의 관리는 실제로 캐시를 구현한 각 계층이 담당한다. 관리 방법 또한 구현에 따라서 다르지만, 여기서는 보편적인 몇 개 개념만 살펴보자.

### 캐시의 성능

- 캐시의 성능은 주로 적중률(Hit ratio) 표현한다. 
...
### 캐시 성능과 용량의 트레이드 오프
- 캐시의 용량이 크면 적중률이 올라간다. 캐싱을 해둘 수 있는 데이터가 많아지기 때문이다.
- 그러나 보통 캐시 역할을 하는 메모리는 접근 속도가 빠른 대신 용량이 제한적이다. 캐시는 임시 저장소이고, 원본 데이터의 일부만 들어간다. 
- 따라서 캐시의 용량이 일정 수준을 넘어가면, 필요없는 캐시를 지워주어야 한다. 용량을 적절히 관리해야 한다.

- 캐시 데이터는 언제든지 삭제될 수 있다는 뜻이다. 말 그대로 임시 보관소이기 때문이다.



### 캐시 교체 알고리즘
- 그렇다면 용량이 부족할 때 어떤 데이터부터 삭제해야할까?
- '캐시 교체 알고리즘'으로 결정한다. 
- 각 메모리 계층별로 구현과 성능이 다양한 캐시 교체 알고리즘이 사용된다. 결국 어떤 데이터를 더 우선순위에 둘 건지를 결정하는 알고리즘이라는 점에서는 다 비슷하다.
- 대표적인 3가지만 알아보도록 하자. 
...
- 실제로 시간적 지역성을 가장 잘 반영하기 때문에 성능이 높은 편이다.
...
- 임시 보관소에 있는 복사본을 사용하면, 한가지 더 골치아픈 일이 생긴다. 
- 다른 프로세스나 스레드에서 Disk에 있는 파일을 수정했는데, 캐시에는 그 수정이 반영이 안된다면 어떨까? 그러면 원본이 바뀌었는데도 캐시 데이터를 여전히 참조해서 일관성 없는 결과를 내놓는다. 
- 즉, 원본 데이터에 변경이 생기면, 느린 저장소에 있는 원본 데이터와 캐시 저장소에 있는 데이터를 일치시켜야한다. 
- 원본 데이터에 변경이 있는 경우 캐시 데이터와 일치시키는 메커니즘을 '캐시 무효화'라고 한다.
- 아니면 하위 계층에 데이터가 변경이 있는지 물어보거나, 일정 주기를 기준으로 캐시를 삭제해주는 방법등이 있다. 
- 직접 사용자가 삭제를 할 수도 있다. 흔히 웹브라우저 오류 해결을 하다가, 잘 안 될 때 '캐시 파일을 삭제해보세요~'라는 말을 볼 수 있다. 설정을 변경했는데 캐시에 이전 데이터가 저장되어있으면, 변경된 설정이 반영 안될 수 있기 때문이다.
...
- 저장 매체의 트레이드오프와 프로그램의 지역성이라는 근본 특성으로 인해서, 메모리 계층이 생기게 된다. 
